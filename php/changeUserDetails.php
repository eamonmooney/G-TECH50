...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    $sql0 = "UPDATE Users SET Name = REPLACE(:oldUsername, :oldUsername, :newUsername)";
    $replacestmt0 = $db->prepare($sql0);
    $replacestmt0->execute([':oldUsername'=> $uData['Name'],
        ':newUsername' => $Username]);
    $sql1 = "UPDATE Users SET Email = REPLACE(:oldEmail, :oldEmail, :newEmail)";
    $replacestmt1 = $db->prepare($sql1);
    $replacestmt1->execute([':oldEmail'=> $uData['Email'],
        ':newEmail' => $Email]);
    $sql2 = "UPDATE Users SET Password = REPLACE(:oldPassword, :oldPassword, :newPassword)";
