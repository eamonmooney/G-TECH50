...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'place_order') {
    ...
    try {
        ...
        if ($userID === 0) {
            ...
            // TEMP VARIABLES - TAKE FROM CHECKOUT FORM
            $name = "Guest A";
            ...
            $GuestInfostmt->execute([$name, $email]);
            // Get the last inserted guestID
            $guestID = $db->lastInsertId();

            // Insert into Orders table as a guest order
            $query = "INSERT INTO Orders (UserID, GuestID, OrderTypeID, OrderDate, OrderCost) VALUES (?, ?, ?, ?, ?)";
            $stmt = $db->prepare($query);
            $stmt->execute([NULL, $guestID, $orderTypeID, $orderDate, $total]);
